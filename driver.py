...
# Run options
RUN_MODE = 'silent'
TTS_ENABLED = not RUN_MODE=='silent'
MODE = 'TEXT' if RUN_MODE=='silent' else 'SPEECH' # TEXT / SPEECH | Text Mode: You write the prompt in the terminal. Speech Mode: You speak the prompt outloud.
...
import llm,llm.run_ollama,llm_parser,llm_parser.function_parser,timer # my libraries
...
def handleResponse(response:llm.requests.Request):
    ...
    """
    print('\nProcessing Response...')
    keywords = ['app','open','timer','searching','airplay','screen mirror','boot','load','launching','code project','coding']
    if True in [keyword in response_text.lower() for keyword in keywords]:
        ...
        for response in parsed_response:
            ...
            if action == 'code_project':
                ...
                path=os.path.join(os.path.expanduser('~'),'Desktop','Code',str(project_name))
